...
class StructuredOutput(BaseModel):
    ...
    final_answer: Optional[str] = Field(None, description="this should be never null, it should be the final answer to the user query, it should be in string format -> it should be the answer to the user query based on the output you got from the tool or it should be your final answer to the user query if you have not used any tool to solve the user query. atleast there should be some meaningful string in this field, if you don't know the answer to the user query, then it should be 'I don't know the answer to this question.")
    ...
    is_final: bool = Field(..., description="if you have provided the final answer to the user query, then it should be true, otherwise it should be false")
